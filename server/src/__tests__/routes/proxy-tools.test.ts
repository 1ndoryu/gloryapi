...
async function request<T = CompletionResponse>(app: Express, method: string, path: string, body?: unknown, headers: Record<string, string> = {}) {
...
describe('Proxy tool-calling support', () => {
  ...
  it('passes tools/tool_choice to provider and returns tool_calls', async () => {
    ...
    let providerBody: ProviderRequestBody;

    vi.spyOn(global, 'fetch').mockImplementation(async (url, init) => {
      const urlStr = typeof url === 'string' ? url : url.toString();
      if (urlStr.includes('api.groq.com/openai/v1/chat/completions')) {
        providerBody = JSON.parse(String(init?.body)) as ProviderRequestBody;
        return {
          ok: true,
  ...
  it('preserves reasoning_content and normalizes assistant null content for OpenCode Zen follow-up turns', async () => {
    const db = getDb();
    db.prepare('DELETE FROM api_keys').run();
    ...
    expect(status).toBe(200);
    expect(providerBody.messages[1].role).toBe('assistant');
    expect(providerBody.messages[1].content).toBe('');
    expect(providerBody.messages[1].reasoning_content).toBe('hidden chain of thought');
    expect(providerBody.messages[1].tool_calls).toHaveLength(1);
  });
});
