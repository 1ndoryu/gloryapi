...
analyticsRouter.get('/history', (req: Request, res: Response) => {
  ...
  const rows = db.prepare(`
    SELECT
      ...
      r.parent_request_id,
      r.parent_route_id,
      r.parent_configuration_revision,
      r.parent_selection_reason,
  ...
  res.json(rows.map(r => ({
    ...
    parentConfigurationRevision: r.parent_configuration_revision,
    parentSelectionReason: r.parent_selection_reason,
