...
export function getBridgeCatalogSyncStatus(projection: Omit<BridgeCatalogProjection, 'sync'>): BridgeCatalogSyncStatus {
  ...
  if (!fs.existsSync(catalogPath)) {
    return { state: 'missing', path: catalogPath, checkedAt, revision: null, hash: null, visionHash: null, errors: ['No existe una proyección local del bridge en la ruta configurada'] };
  }
  try {
    const file = JSON.parse(fs.readFileSync(catalogPath, 'utf8')) as { schemaVersion?: string; revision?: number; hash?: string; visionHash?: string; entries?: unknown[]; visionModels?: unknown[] };
    const errors: string[] = [];
    ...
    if (!Array.isArray(file.visionModels)) errors.push('visionModels local no es un array');
    if (typeof file.visionHash !== 'string' || !file.visionHash) errors.push('visionHash local inválido');
    ...
    if (file.visionHash !== projection.visionHash) errors.push('visionHash local no coincide con la proyección canónica');
    return {
      ...
      visionHash: typeof file.visionHash === 'string' ? file.visionHash : null,
      errors,
    };
  } catch (error) {
    return { state: 'invalid', path: catalogPath, checkedAt, revision: null, hash: null, visionHash: null, errors: [error instanceof Error ? error.message : 'No se pudo leer la proyección local'] };
  }
}
...
export function getBridgeCatalogProjection(): BridgeCatalogProjection {
  ...
  const visionModels = getBridgeVisionModels();
  const visionHash = crypto.createHash('sha256').update(JSON.stringify(visionModels)).digest('hex');
  return { schemaVersion: 'glory-bridge-model-catalog-v2', revision, hash, visionHash, generatedAt: new Date().toISOString(), entries, visionModels };
}
