...
/* [2076-16] Proactive rate limit threshold check.
 * Returns true if usage is at or above the given threshold (default 80%) of any limit.
 * Used by the router to deprioritize models before they actually hit the limit,
 * avoiding 429 errors and wasted retry attempts. */
export function isNearLimit(
  platform: string,
  modelId: string,
  keyId: number,
  limits: { rpm: number | null; rpd: number | null; tpm: number | null; tpd: number | null },
  threshold = getSettingNumber('routing.nearLimitThreshold'),
): boolean {
  const now = Date.now();
  ...
  if (limits.rpm !== null) {
    ...
    const w = getWindow(key);
    w.timestamps = pruneTimestamps(w.timestamps, MINUTE, now);
    if (w.timestamps.length >= limits.rpm * threshold) return true;
  }
  ...
  if (limits.rpd !== null) {
    ...
    const w = getWindow(key);
    w.timestamps = pruneTimestamps(w.timestamps, DAY, now);
    if (w.timestamps.length >= limits.rpd * threshold) return true;
  }
  ...
  if (limits.tpm !== null) {
    ...
    w.tokenTimestamps = w.tokenTimestamps.filter(t => t.ts > now - MINUTE);
    const used = w.tokenTimestamps.reduce((sum, t) => sum + t.tokens, 0);
    if (used >= limits.tpm * threshold) return true;
  }
  ...
  if (limits.tpd !== null) {
    ...
    w.tokenTimestamps = w.tokenTimestamps.filter(t => t.ts > now - DAY);
    const used = w.tokenTimestamps.reduce((sum, t) => sum + t.tokens, 0);
    if (used >= limits.tpd * threshold) return true;
  }
