...
6. **E2E Desktop/proveedor real — parcialmente completada.** Con ChatGPT intacto, los helpers locales resolvieron las credenciales en memoria y un bridge temporal en `:4197` contra GloryAPI `:3101` pasó readiness, texto, `function_call` y SSE, sin exponer `FALLBACK_REASONING`; el proceso fue detenido y limpiado. Sigue pendiente el E2E desde la aplicación Desktop y la atribución verificable a un proveedor individual; la capability continúa `unverified`.
