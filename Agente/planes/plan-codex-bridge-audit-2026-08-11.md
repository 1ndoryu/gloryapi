...
4. **Canary de tres proveedores — completada.** El harness provisiona SQLite/puertos/credenciales temporales, helper DPAPI token-only, arranca los dos runtimes y cierra conexiones keep-alive de forma bounded antes del cleanup; la selección server-side autenticada ejecuta una petición Responses directa por proveedor más fallback; `auto`, modelos no declarados y token inválido quedan fail-closed.
5. **Verificación — completada localmente.** `npm run build:server`, `npm test -w server` (270 tests en 47 archivos), 96 tests raíz + 2 tests de seguridad, `npm run task:check -- GLORY-BASELINE` y `npm run canary:codex` pasan; el canary ejecuta SSE Responses real, continuidad al cambiar entre los tres proveedores con historial verificado, continuidad de una llamada de herramienta al cambiar de Andoryyu a Go, ciclo aislado `codex app-server` JSONL con dos turnos por `turn.id`, ejecución de `shell_command` y compaction observable, toolset namespaced de plugin/MCP, forwarding aislado del Browser skill desde el marketplace local bundled con sanitización allowlist y fuente fija, ronda multi-agente con `agent_message` y resultado de herramienta, proyector bounded/allowlisted de trazas live, aislamiento de entorno del auditor live, cancelación bounded de respuestas no-SSE, aislamiento de entorno, stop seguro de PID stale y además un `shell_command` real desde Codex CLI temporal. El bridge se detiene al finalizar. El smoke externo de OpenCode Zen se auto-omite ante `401`/modelo no soportado y no se usa como evidencia de salud externa.
...
7. **Coexistencia de historiales — completada localmente.** Se preparó `%USERPROFILE%\\.codex-gloryapi` como segundo `CODEX_HOME` para el bridge, sin copiar `auth.json`, SQLite ni conversaciones desde `%USERPROFILE%\\.codex`. `start-codex-bridge.ps1` usa un perfil configurable y splatting nombrado de PowerShell; los tests controlados verifican `CODEX_HOME` y `--profile` en CLI/Desktop. `switch-chatgpt.ps1` no muta el home normal por defecto; la restauración global antigua requiere `-LegacyGlobalConfig`. El bridge quedó detenido y el E2E real Desktop continúa fuera de alcance.
...
- [x] ChatGPT normal y bridge tienen homes/historiales separados; los switches seguros no mutan el home normal.
