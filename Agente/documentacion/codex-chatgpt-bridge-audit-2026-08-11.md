...
|---|---|---|---|---|
| Andoryyu | OpenAI-compatible | El gateway bufferiza hasta `[DONE]` porque el worker puede truncar streams en cualquier punto. | PASS directo; PASS como primer candidato de fallback. | Observado en la ventana live: `429` no-stream y `response.failed` en SSE; primer candidato de fallback normal. |
| OpenCode Zen | OpenAI-compatible | Normaliza `content: null` y asegura `reasoning_content` en assistant turns de thinking. | PASS directo; PASS como fallback determinista de Andoryyu. | Observado en la ventana live: `429` no-stream y `response.failed` en SSE; no se declara sano de forma estable. |
| OpenCode Go | OpenAI-compatible | Comparte la normalización de reasoning de Zen; es el último recurso configurado. | PASS directo; incluido en la cadena y en capabilities del gateway. | Observado en la ventana live: `200` no-stream/SSE; `message` y `response.completed`; destino del fallback normal. |

“PASS directo” significa una petición Responses completa a través del bridge, con el gateway forzado server-side al proveedor correspondiente y un upstream OpenAI-compatible local determinista. La columna live registra una observación de una ventana concreta, no un SLA ni salud simultánea: los `429` de Andoryyu/Zen son disponibilidad o cuota de esa ventana, mientras Go sí respondió. Por ello `/capabilities.providerInference` continúa `unverified` fuera del canary y no se atribuye estabilidad permanente a ningún proveedor.
...
- `node --test integrations/codex-bridge/test/*.test.cjs`: **96/96 PASS**; `node --test integrations/codex-bridge/test/security/*.test.cjs`: **2/2 PASS**. En conjunto son 98 pruebas: anti-falso-complete, reasoning-only, tool-only, web loop, compactación de seguridad con schemas de herramientas, resumen configurable, UTF-8 fragmentado, truncamiento, cancelación, preflight, perfiles de herramientas, toolset namespaced de plugin/MCP, ronda multi-agente `function_call` → `agent_message` → `function_call_output`, continuidad de `function_call` → `function_call_output` al cambiar de Andoryyu a Go con rechazo de llamada ausente o reordenada, proyector de trazas live bounded/allowlisted, allowlist de entorno y cleanup acotado del auditor live, continuidad incompleta/reordenada rechazada, timers de visión limpiados en éxito/fallo, redacción de visión, límites de body, routing canary preservado en reintentos, streams que quedan abiertos después de headers, sanitización allowlist del perfil de plugins con variantes de secretos/fuentes remotas/rutas no canónicas rechazadas, aislamiento de entorno y stop seguro de PID stale.
- `npm run build:server`: **PASS**.
- `npm test -w server`: **270/270 PASS** en 47 archivos, incluidos los tres casos de routing canary fail-closed. La primera ejecución dentro del sandbox no pudo resolver esbuild (`Access is denied` al resolver `../../../../..`); una segunda ejecución local fuera de esa restricción terminó correctamente. El smoke live de OpenCode Zen se auto-omitió porque el endpoint respondió `401`/modelo no soportado, sin convertirlo en un falso PASS.
- `npm run canary:codex`: **PASS**. Resultado: `readiness`, `lifecycle`, `capabilities`, texto, SSE Responses real (`stream`), continuidad al cambiar entre proveedores (`providerSwitching`), continuidad de `function_call` en Andoryyu a `function_call_output` y respuesta final en OpenCode Go (`providerToolSwitching`), ciclo aislado `codex app-server` JSONL con `thread/start`, dos `turn/start` asociados por `turn.id`, `turn/completed`, ejecución de `shell_command` y `thread/compact/start` (`appServer`, `appServerToolExecution`, `appServerCompaction`), toolset namespaced de plugin/MCP (`pluginTooling`), forwarding aislado del Browser skill con `features.plugins=true`, instalación temporal desde el marketplace local y marcadores distintivos de sus instrucciones (`pluginSkillForwarding`), loop web interno, ejecución real de `shell_command` desde Codex CLI en `CODEX_HOME` temporal (`codexToolExecution`), fallback, foreign toolset sin cooldown, aislamiento y `providerCoverage` directo para `andoryyu`, `opencode-zen` y `opencode-go`.
...
- La provisión aislada no requiere una DB persistente: `npm run canary:codex` crea SQLite, `CODEX_HOME`, tokens y puertos temporales. Tras corregir el cierre keep-alive del upstream, una ejecución verificada terminó en 28,5 s con código 0 y eliminó el runtime temporal.
