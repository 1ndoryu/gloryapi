...
**One OpenAI-compatible endpoint. Twenty-six free LLM providers. ~1B+ tokens per month.**

Aggregate the free tiers from Google, Groq, Cerebras, SambaNova, NVIDIA, Mistral, OpenRouter, GitHub Models, Cohere, Cloudflare, HuggingFace, Z.ai (Zhipu), OpenCode Zen, OpenCode Go, Pollinations, LLM7, Ollama Cloud, Kilo Gateway, CommandCode, SiliconFlow, BazaarLink, Hyperbolic, Scaleway, Morph, PublicAI, and SenseNova behind a single `/v1/chat/completions` endpoint. Keys are stored encrypted. A router picks the best available model for each request, falls over to the next provider when one is rate-limited, and tracks per-key usage so you stay under every free-tier cap.

[![CI](https://github.com/tashfeenahmed/gloryapi/actions/workflows/ci.yml/badge.svg)](https://github.com/tashfeenahmed/gloryapi/actions/workflows/ci.yml)
...
The problem is that stacking them by hand is painful: fourteen different SDKs, fourteen different rate limits, fourteen places a request can fail. GloryAPI collapses that into one OpenAI-compatible endpoint. Point any OpenAI client library at your local server, and it routes transparently across whichever providers you've added keys for.
...
<td align="center" width="180"><a href="https://groq.com"><b>Groq</b><br/><i>Disabled — invalid key</i></a></td>
...
<tr>
<td align="center"><a href="https://ollama.com"><b>Ollama Cloud</b><br/>GLM-4.7 · Kimi K2 · Granite 4</a></td>
<td align="center"><a href="https://kilo.ai"><b>Kilo Gateway</b><br/>Aggregator (200 req/hr)</a></td>
<td align="center"><a href="https://commandcode.ai"><b>CommandCode</b><br/>DeepSeek V4 Flash (paid proxy)</a></td>
<td align="center"><a href="https://siliconflow.cn"><b>SiliconFlow</b><br/>Qwen3 · DeepSeek V3 (auth required)</a></td>
</tr>
<tr>
<td align="center"><a href="https://bazaarlink.ai"><b>BazaarLink</b><br/>DeepSeek V4 · Qwen 3.7 · GLM-5 · Kimi K3</a></td>
<td align="center"><a href="https://hyperbolic.xyz"><b>Hyperbolic</b><br/>Open-source models (auth required)</a></td>
<td align="center"><a href="https://scaleway.com"><b>Scaleway</b><br/>1M free tokens · European</a></td>
<td align="center"><a href="https://morphllm.com"><b>Morph</b><br/>250K credits/mo free</a></td>
</tr>
<tr>
<td align="center"><a href="https://sensenova.cn"><b>SenseNova</b><br/>1500 calls/5h per model (free beta)</a></td>
<td></td>
<td></td>
<td></td>
</tr>
...
- **Unified API key** — Clients authenticate to your proxy with a single `gloryapi-…` bearer token. You never expose upstream provider keys to your apps.
- **Health checks** — Periodic probes mark keys as `healthy`, `rate_limited`, `invalid`, or `error` so the router skips dead ones automatically. Provider-level cooldown: 3 consecutive failures → 60s cooldown, auto-recovery on success.
- **Smart routing** — Arena ELO-based model ranking, proactive 80% rate limit avoidance, two-pass fallback (prefer less-loaded models first, then relaxed pass to avoid 429s).
...
```bash
git clone https://github.com/tashfeenahmed/gloryapi.git
cd gloryapi
npm install
...
```bash
npm run build
node server/dist/index.js     # server + dashboard both served on :3101
```
...
client = OpenAI(
    ...
    api_key="gloryapi-your-unified-key",
)
...
curl http://localhost:3101/v1/chat/completions \
  -H "Authorization: Bearer gloryapi-your-unified-key" \
...
```bash
npm install
npm run dev      # server on :3101, dashboard on :5173, both with HMR
...
Free catalog additions tracked in the V23 migration: the OpenCode Zen `minimax-m3-free` model is now reachable through the router as `minimax-m3-free` on the `opencode-zen` platform. Confirmed live against the Zen `/v1/chat/completions` endpoint on 2026-06-01 (anonymous, 200 OK, `model="MiniMax-M3"`, `cost=0`). The provider does not document this model on its public model list, so the catalog row is keyed off the `/v1/models` discovery probe rather than the docs.

## Disclaimer

**This project is for personal experimentation and learning, not production.** Free tiers exist so developers can prototype against them; they aren't a stable, supported inference substrate and shouldn't be treated as one. If you build something real on top of GloryAPI, swap in a paid API before you ship. Your relationship with each upstream provider is governed by the terms you accepted when you created your account — those terms still apply when the traffic is proxied through this project, and you're responsible for complying with them.

## Star History

[![Star History Chart](https://api.star-history.com/chart?repos=tashfeenahmed/gloryapi&type=date&legend=top-left)](https://www.star-history.com/?repos=tashfeenahmed%2Fgloryapi&type=date&legend=top-left)
